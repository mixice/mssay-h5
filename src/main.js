...
const TAU = PI * 2;
let time = new Date;
const lerp = (start, end, amt) => {return (1-amt)*start+amt*end};
const distance = (x1, y1, x2, y2) => {const a = x1 - x2;const b = y1 - y2;return Math.sqrt( a*a + b*b );};
const angle = (cx, cy, ex, ey) => {return Math.atan2(ey - cy, ex - cx);};
const particlePrototype = () => ({
	...
	color: {
		r: Math.round(300 + r(55)),
		g: Math.round(280 + r(105)),
		b: Math.round(300 + r(55))
	}
});
...
const update = () => {
    particles.forEach(p1 => {
		...
		if (p1.y > h || p1.y < 0) {p1.angle = -p1.angle;}
